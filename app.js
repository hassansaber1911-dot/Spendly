...
function toggleCat(c){let key=c.replace(/\s/g,"-"),el=document.getElementById("cat-"+key),ic=document.getElementById("icon-"+key);if(!el)return;el.hidden=!el.hidden;if(ic)ic.textContent=el.hidden?"+":"−"}
...
function saveBudgets(){state.budgets[month]=state.budgets[month]||{};document.querySelectorAll(".budget-input").forEach(i=>state.budgets[month][i.dataset.cat]=Number(i.value||0));save();budgetEditing=false;render()}
...
function addIncome(){
 ...
 state.incomes=state.incomes||[];state.incomes.push({id:uid(),date,amount,source});save();render()
}
...
function saveIncomeEdit(id){
 ...
 if(!date||!amount)return alert("Date and amount are required.");
 x.date=date;x.amount=amount;x.source=document.querySelector("#eisource").value.trim();save();modal={kind:"income"};render()
}
function deleteIncome(id){if(confirm("Delete this income?")){state.incomes=(state.incomes||[]).filter(x=>x.id!==id);save();modal={kind:"income"};render()}}
...
function saveTx(existing){let amount=Number(document.querySelector("#amount").value),date=document.querySelector("#date").value;if(!amount||!date)return alert("Date and amount are required.");let x={id:existing||uid(),type:modal.type,date,amount,note:document.querySelector("#note").value.trim()};if(x.type==="Expense"){x.category=document.querySelector("#cat").value;x.subcategory=document.querySelector("#sub").value.trim();state.subcats[x.category]=state.subcats[x.category]||[];if(x.subcategory&&!state.subcats[x.category].includes(x.subcategory))state.subcats[x.category].push(x.subcategory)}else{x.subcategory=document.querySelector("#savetype").value.trim()||"Savings";if(!state.subcats.Savings.includes(x.subcategory))state.subcats.Savings.push(x.subcategory)}let i=state.tx.findIndex(a=>a.id===existing);if(i>=0)state.tx[i]=x;else state.tx.push(x);save();modal=null;render()}
function deleteTx(i){if(confirm("Delete this transaction?")){state.tx=state.tx.filter(x=>x.id!==i);save();modal=null;render()}}
function onboarding(){if(state.profile.onboarded)return "";if(!state.profile.name)return `<div class="modal"><div class="sheet"><h2>Welcome 👋</h2><p class="muted">What should we call you?</p><div class="field"><label>Your name</label><input id="on"></div><button class="btn" onclick="let n=document.querySelector('#on').value.trim();if(n){state.profile.name=n;save();render()}">Continue</button></div></div>`;return `<div class="modal"><div class="sheet"><h2>Quick setup</h2><div class="tour"><b>1. Add your income 💰</b><div class="small muted">Set the money available for the month.</div></div><div class="tour"><b>2. Add expenses by date</b><div class="small muted">Choose date, amount, category and subcategory.</div></div><div class="tour"><b>3. Track Savings separately</b><div class="small muted">Savings reduce available balance but do not count as expenses.</div></div><div class="tour"><b>4. Set category budgets</b><div class="small muted">See planned vs actual spending.</div></div><button class="btn" onclick="state.profile.onboarded=true;save();modal={kind:'income'};render()">Start tracking</button></div></div>`}
